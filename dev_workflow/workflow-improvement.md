...
## Improvement loop
1. Identify improvement candidates in the active feedback record in `intent/` or the active `dev_log/` files.
...
5. Record the improvement in `dev_log/design-update-log.md` with a `DEV-*` reference.

## Outputs
- A proposed template improvement summary.
- Updated workflow or template files with clear change rationale.
- A note in `dev_log/design-update-log.md` linking the improvement to the relevant `DEV-*`.
