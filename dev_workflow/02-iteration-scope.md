...
## Purpose
Define the exact work for the next iteration by interpreting the latest plan and dev logs into updated design scope and proof requirements.
...
- `dev_log/change-log.md` if interpreting plan changes current scope
...
6. Make sure each `ACC-*` has a proving path through `TEST-*` or manual review.
