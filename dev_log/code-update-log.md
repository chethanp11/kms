...
- `DEV-022` | `2026-05-12` | Added `.env.example`, ignored local `.env` files, AI settings with hidden API-key repr, a stdlib OpenAI Responses API client, and GPT-4o-backed knowledge-understanding extraction with deterministic fallback. | Enable server-side API-key-backed AI activities without committing secrets or granting AI direct publish authority. | `REQ-022`, `DEV-022`, `TEST-022` | `user prompt`, `intent/product-intent.md`
- `DEV-023` | `2026-05-12` | Added candidate workflow runtime/API methods for create, approve, and publish; rewired KMI to a three-stage candidate flow; removed create-demo-source UI behavior; beautified KMI and Infopedia presentation. | Implement the requested KMI workflow while preserving approval and `/wiki` publication boundaries. | `REQ-023`, `DEV-023`, `TEST-023` | `user prompt`, `intent/product-intent.md`
