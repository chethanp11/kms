...
- `DEV-016` | `2026-05-10` | Added all-component source scaffold guidance to `design/architecture.md`. | The user requested scaffolding for all components; section 3.3 provides the approved component set and authority boundaries. | `REQ-016`, `DEV-016`, `TEST-016` | `user prompt`
- `DEV-017` | `2026-05-10` | Clarified target folder blank scaffold expectations in `design/architecture.md` and excluded `src/components` from approved scaffold shape. | The user clarified scaffolding should be blank files under target design folders, not metadata component packages. | `REQ-017`, `DEV-017`, `TEST-017` | `user prompt`
- `DEV-018` | `2026-05-10` | Added corrected `src/`-only runtime scaffold guidance to `design/architecture.md`. | The user clarified that KMS application code scaffolding should be empty/minimal files inside `src/` only, not external target folders or `src/components` metadata packages. | `REQ-018`, `DEV-018`, `TEST-018` | `user prompt`
- `DEV-019` | `2026-05-11` | Completed `design/architecture.md` section 9.5 field definitions for SourceDocument, ImpactRecord, QAReport, LintFinding, InfopediaNode, and SearchDocument while preserving `/wiki` authority and projection boundaries. | The current prompt requested complete core KMS domain contracts based on architecture section 9. | `REQ-019`, `DEV-019`, `TEST-019` | `user prompt`, `intent/product-intent.md`
- `DEV-020` | `2026-05-11` | No design document change; interpreted existing KMS architecture and acceptance criteria as sufficient for populating the `src/` runtime scaffold. | The current prompt asked to implement code according to existing design, and no new behavior or boundary was required. | `REQ-020`, `DEV-020`, `TEST-020` | `user prompt`, `intent/product-intent.md`
