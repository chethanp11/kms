...
| `intent/product-intent.md` | API-backed AI extraction configuration | `REQ-022` | OpenAI GPT-4o extraction uses server-side `OPENAI_API_KEY` config, validates responses into candidate contracts, falls back deterministically on missing key or API failure, and never exposes secrets or grants publish authority | `TEST-022` | `DEV-022` | Mocked API coverage lives in `tests/unit/test_knowledge_understanding.py`; local secrets are ignored through `.gitignore` |
