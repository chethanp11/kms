...
- `evals/`: AI-specific assessments that may be automated, manual, or hybrid.
- `fixtures/`: reusable inputs, golden outputs, and evaluation samples.

## Working rules
- `tests/` is system-managed and should be updated through the intent -> design -> validation workflow.
- Every changed behavior should point to at least one `TEST-*`, `EVAL-*`, or documented manual review step.
...
- When an eval acts like a release gate, record evidence in `dev_log/validation-log.md`.
