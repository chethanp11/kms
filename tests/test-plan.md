...
| `TEST-022` | `unit/static` | Validate API-key-backed GPT-4o configuration, mocked OpenAI candidate extraction, deterministic fallback on API failure, ignored secret placeholders, and no live network dependency in tests. | `REQ-022`, `DEV-022` | `tests/unit/test_knowledge_understanding.py`, unit discovery, Python compile, `.gitignore`/`.env.example` review | Platform engineer |
| `TEST-023` | `unit/static` | Validate three-stage KMI candidate workflow: create candidates, approve all, publish approved candidates to `/wiki`, and verify Infopedia tree/search visibility plus frontend stage controls. | `REQ-023`, `DEV-023` | `tests/unit/test_working_kms_runtime.py`, `tests/unit/test_src_runtime_scaffold.py`, unit discovery, Python compile | Platform engineer |
| `TEST-024` | `unit/static` | Validate semantic candidate decomposition, duplicate auto-rejection visibility, per-candidate approve/reject/approve-with-mods decisions, source-path publication, and confidence-scored Infopedia search. | `REQ-024`, `REQ-025`, `REQ-026`, `REQ-027`, `DEV-024` | `tests/unit/test_working_kms_runtime.py`, `tests/unit/test_knowledge_understanding.py`, `tests/unit/test_src_runtime_scaffold.py`, unit discovery, Python compile | Platform engineer |
